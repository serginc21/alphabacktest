...
- **TA-lib**. As there are sometimes difficulties when installing this module, there is an option for the backtest not to calculate any technical indicator (as all are based on TA-lib), so in that case you don't necessarily need to have it installed.
- **Dash**. The package uses dash in order to plot and display the results. However, this option can also be desactivated.
